...
@section('content')
    <div class="mx-auto grid h-full max-w-7xl grid-rows-1 px-5 pt-5 lg:grid-cols-7 xl:px-0">
        <div class="col-span-7">
            <p class="mb-5 text-base sm:text-lg md:text-xl"><a href="{{route('forum.topics.all',['id'=>$topic->subcategory->id])}}">{{$topic->subcategory->title}}</a></p>
            <p class="text-base sm:text-2xl md:text-5xl">{{$topic->title}}</p>
            <p class="text-sm NunitoSans sm:text-xl md:text-2xl">{{$topic->text}}</p>
            <p class="mb-10 text-sm NunitoSans md:text-lg">{{$topic->created_at}}</p>
        </div>
